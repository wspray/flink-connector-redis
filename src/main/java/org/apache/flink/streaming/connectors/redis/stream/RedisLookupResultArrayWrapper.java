...
public class RedisLookupResultArrayWrapper {
    ...
    public static List<Row> createRowDataForRow(String value,
        ...
        for (Map<String, Object> jsonMap : jsonMapList) {
            ...
            rows.add(RedisLookupResultWrapper.createRowDataForRow(jsonValue, dataTypes));
        }
        return rows;
    }
        ...
        if (redisValueDataStructure == RedisValueDataStructure.column) {
            map.forEach((field, value) -> {
                if (valueTypeJson && isJsonArray(value)) {
                    ...
                    Row row = RedisLookupResultWrapper.createRowDataForRow(value, dataTypes);
                    row.setField(
                            KEY,
                            RedisRowConverter.dataTypeFromStringWithException(
                                    Types.STRING, String.valueOf(keys[0])));
