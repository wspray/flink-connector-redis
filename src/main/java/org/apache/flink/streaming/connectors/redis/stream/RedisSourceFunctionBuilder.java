...
public class RedisSourceFunctionBuilder<T> {
    ...
    private void inferRowTypeInfo(RedisCommand redisCommand) {
        ...
        List<String> valueFieldNames = new ArrayList<>(keySet);
        List<TypeInformation> valueTpeNames = new ArrayList<>(typeMap.size());
        for (String valueFieldName : valueFieldNames) {
            valueTpeNames.add(typeMap.get(valueFieldName));
        }
        rowTypeInfo = new RowTypeInfo(valueTpeNames.toArray(new TypeInformation[0]), keySet.toArray(new String[0]));
    }

    private TypeInformation<Row> mergeTypeInformation(RowTypeInfo rowTypeInfo,
                                                      RowTypeInfo valueTypeInfo) {
        TypeInformation<?>[] fieldTypes = rowTypeInfo.getFieldTypes();
        String[] fieldNames = rowTypeInfo.getFieldNames();
        return new RowTypeInfo(
                ArrayUtils.addAll(fieldTypes, valueTypeInfo.getFieldTypes()),
                ArrayUtils.addAll(fieldNames, valueTypeInfo.getFieldNames())
        );
    }
}